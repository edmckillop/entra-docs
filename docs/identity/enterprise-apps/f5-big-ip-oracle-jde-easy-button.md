...
#customer intent: I'm an IT admin responsible for securing Oracle JD Edwards (JDE). I want to configure F5 BIG-IP Easy Button for single sign-on (SSO) to Oracle JDE using Microsoft Entra ID, so I can improve the application's security posture and enable SSO between Microsoft Entra ID and BIG-IP published services.
---
